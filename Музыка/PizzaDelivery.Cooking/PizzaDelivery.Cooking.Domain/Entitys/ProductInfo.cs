...
public class ProductInfo : Entity<Guid>
{
    ...
    public Result<ProductInfo> Create(Guid id, string name, List<ProductContaining> containings)
    {
        if (string.IsNullOrWhiteSpace(name))
        {
            return Result.Failure<ProductInfo>("product name must be");
        }

        if (containings is null || containings.Count == 0)
        {
            return Result.Failure<ProductInfo>("product must have ingridient");
        }
        
        return Result.Success<ProductInfo>(new ProductInfo(id, name, containings));
    }

    protected ProductInfo(Guid id, string name, List<ProductContaining> containings)
    {
        Id = id;
        Name = name;
