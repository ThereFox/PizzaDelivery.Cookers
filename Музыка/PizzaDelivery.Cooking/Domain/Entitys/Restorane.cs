...
public class Restorane : Entity<Guid>
{
    ...
    public Restorane(Guid id, WorkTime workTime, Address address, List<RestoraneStocks> stocks, List<Cooker> cookers)
    {
        Id = id;
        WorkTime = workTime;
        Address = address;
    ...
    public static Result<Restorane> Create(Guid id, WorkTime workTime, Address address, List<RestoraneStocks> stocksList, List<Cooker> workers)
    {
        if (workers.Any() == false)
        {
            return Result.Failure<Restorane>("restorane must contain workers");
        }

        return Result.Success<Restorane>(new (id, workTime, address, stocksList, workers));
